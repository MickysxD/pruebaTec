...
querys.obtenerProyectos = async function obtenerProyectos() {
    return new Promise(async (resolve, reject) => {
        await db.ejecutarQuery(`CALL obtenerProyectos();`)
            .then(res => {
                let response = {
                    message: 'Consulta exitosa',
                    show: false,
...
querys.actualizarProyecto = async function actualizarProyecto(data) {
    return new Promise(async (resolve, reject) => {
        await db.ejecutarQuery(`CALL actualizarProyecto(${data.id},'${data.nombre}','${data.municipio}','${data.departamento}','${data.fecha_inicio}','${data.fecha_fin}');`)
            .then(res => {
                let response = {
                    message: 'Consulta exitosa',
                    show: false,
...
querys.eliminarProyecto = async function eliminarProyecto(data) {
    return new Promise(async (resolve, reject) => {
        await db.ejecutarQuery(`CALL eliminarProyecto(${data.id});`)
            .then(res => {
                let response = {
                    message: 'Consulta exitosa',
                    show: false,
